...
export default class Time extends Component {
  ...
  isEarlierThanEndLimit = (timeValue, endLimit, lastValue) => {
    let timeValueIsEarlier =
      moment(timeValue, "h:mm").diff(moment(endLimit, "h:mm")) < 0;
    let timeValueIsLaterThanLastValue =
      lastValue === undefined
        ? true
        : moment(lastValue, "h:mm").diff(moment(timeValue, "h:mm")) < 0;
  ...
  render() {
    ...
    while (this.isEarlierThanEndLimit(timeValue, endLimit, lastValue)) {
      lastValue = timeValue;
      timeValue = moment(timeValue, "HH:mm")
        .add(step, "minutes")
        .format("HH:mm");
      options.push(timeValue);
    }
        ...
        {options.map(timeValue => {
          ...
          let exception =
            timeException.filter(
              e => e.startTime <= timeValue && e.endTime >= timeValue
            ).length > 0;

          return (
            <div key={key} className="timeChoice__button">
              <input
                type="radio"
                name={timeFieldName}
                id={key}
              ...
              />
              <label htmlFor={key}>
                {moment(timeValue, "HH:mm").format("hh:mm a")}
              </label>
              <button
                type="button"
                className="timeChoice__confirm"
                onClick={() =>
                  this.props.onConfirmTime(moment(timeValue, "HH:mm").format())
                }
              >
                Confirm
              </button>
